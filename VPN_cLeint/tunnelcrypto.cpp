...
bool TunnelCrypto::deriveClientSessionKeys(const QByteArray &clientPrivateKey,
    ...
    QByteArray sharedSecret;
    if (!deriveSharedSecret(clientPrivateKey, serverPublicKey, &sharedSecret, error)) {
        return false;
    }
    ...
    keys->txKey = derived.mid(0, kKeySize);
    keys->rxKey = derived.mid(kKeySize, kKeySize);
    keys->txNoncePrefix = qFromBigEndian<quint32>(reinterpret_cast<const uchar *>(derived.constData() + 64));
    keys->rxNoncePrefix = qFromBigEndian<quint32>(reinterpret_cast<const uchar *>(derived.constData() + 68));
